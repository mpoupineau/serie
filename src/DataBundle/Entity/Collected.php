 ...
 * @ORM\Entity(repositoryClass="DataBundle\Repository\CollectedRepository")
 */
class Collected
{
    /**
	...
	/**
	* @ORM\ManyToOne(targetEntity="UserBundle\Entity\User", inversedBy="collecteds")
	* @ORM\JoinColumn(nullable=true)
	*/
	private $user;

	/**
	* @ORM\ManyToOne(targetEntity="DataBundle\Entity\Serie", inversedBy="collecteds")
	* @ORM\JoinColumn(nullable=true)
	*/
	private $serie;
    ...
    public function populate($data) {
        dump($data);
        $this->setRated($data['rating']);
        $this->setSeasonSeen($data['seasonSeen']);
        $this->setEpisodeSeen($data['episodeSeen']);
        $this->setComment($data['comment']);
        $this->setFollow(false);
        $this->setAlertEachEpisode(false);
        $this->setAlertFirstEpisode(false);
        $this->setAlertLastEpisode(false);
        if($data['follow'] == "true")
            $this->setFollow(true);
        if($data['alertEachEpisode'] == "true")
            $this->setAlertEachEpisode(true);
        if($data['alertFirstEpisode'] == "true")
            $this->setAlertFirstEpisode(true);
        if($data['alertLastEpisode'] == "true")
            $this->setAlertLastEpisode(true);
    }
