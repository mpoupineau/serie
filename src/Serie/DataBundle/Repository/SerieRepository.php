...
namespace Serie\DataBundle\Repository;
...
class SerieRepository extends \Doctrine\ORM\EntityRepository
{
	public function findSuggestion($user_id)
	{
		$query = 	'
					SELECT s
					FROM SerieDataBundle:Serie s JOIN s.collecteds c JOIN c.user u where u.id <> :user_id
					';
		// $query = 	'
					// SELECT s
					// FROM SerieDataBundle:Collected c JOIN c.user u JOIN c.serie s WHERE u.id <> :user_id
					// ';
	...
	public function findToDelete()
	{
		$query = 	'
					SELECT s
					FROM SerieDataBundle:Serie s WHERE SIZE(s.collecteds) = 0
					';
		$qb = $this->getEntityManager()->createQuery($query);
